...
national <- read.csv("https://github.com/melaniewalsh/Neat-Datasets/raw/main/us-covid.csv")
...
states <- read.csv("https://github.com/melaniewalsh/Neat-Datasets/raw/main/us-states-covid.csv")
...
counties <- read.csv("https://github.com/melaniewalsh/Neat-Datasets/raw/main/us-counties-covid.csv")
...
# Guess: Which state do you think has the highest number of COVID cases, and which state do you think has the lowest?
 ...
 #Its got to be Wyoming since it has the lowest population and small city life
...
# HINT: The dyplr lag() function will be very helpful here.
    ...
    national <- national %>%
      mutate(new_cases = cases - lag(cases, n = 0)) 
...
# HINT: The dyplr lag() function will be very helpful here.
    ...
    national <- national %>% 
      mutate(new_deaths = deaths - lag(deaths, n = 0)) 
...
most_deaths <- -national %>% filter(date == max(date)) %>% pull(deaths)
...
highest_cases_in_each_state <- counties %>%
  group_by(state) %>% filter(date == max(date, na.rm = TRUE)) %>%
  filter(cases == max(cases, na.rm = TRUE))
...
# Q1 Which location had the most amount of deaths during the begining of the pandemic
  
# Q2 Which location had the least amont of deaths during the begining of the pandemic

#  Write code (at least 2-3 lines) that will answer your question
 most_deaths_begining <- counties %>% 
  filter(date == min(date, na.rm = TRUE)) %>% 
  filter(deaths == max(deaths, na.rm = TRUE)) %>%
  pull(location)

least_deaths_begining <- counties %>%
  filter(date == min(date, na.rm = TRUE)) %>%
  filter(deaths == max(deaths, na.rm = TRUE)) %>%
  pull(location)
...
I would have deffinetly done my filtering alot differently. I do feel the same about this project compared to the data wrangling project. 
